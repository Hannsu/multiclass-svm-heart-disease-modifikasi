...
# ─── WARNA TEMA (sesuai PPT merah-krem) ──────────────────────────────────────
RED    = "#C8192C"
CREAM  = "#F5EDE4"
DARK   = "#2B2B2B"
LIGHT  = "#F0B3B8"
...
# ─── 1. LOAD DATASET ─────────────────────────────────────────────────────────
print("=" * 60)
print("  MODIFIKASI 2 – DATASET KAGGLE (heart.csv)")
print("=" * 60)

DATA_PATH = r"C:\Users\ferdi\Downloads\tugas akhir ml teotzy\multiclass-svm-heart-disease-modifikasi\heart.csv"
...
fig.suptitle("Modifikasi 2 – Klasifikasi Penyakit Jantung (Kaggle Dataset)",
...
# ── Plot 6: Feature Importance (dari korelasi dengan target) ──────────────────
...
df[df['target'] == 1]['age'].plot.hist(ax=ax7, bins=15, color=RED, alpha=0.7,
                                       label='Healthy (1)', edgecolor='white')
df[df['target'] == 0]['age'].plot.hist(ax=ax7, bins=15, color=LIGHT, alpha=0.7,
...
# ── Plot 8: Boxplot Kolesterol ────────────────────────────────────────────────
ax8 = fig.add_subplot(gs[2, 1])
data_box = [df[df['target'] == 1]['chol'].values,
            df[df['target'] == 0]['chol'].values]
bp = ax8.boxplot(data_box, labels=['Healthy (1)', 'Sick (0)'],
                 patch_artist=True, medianprops=dict(color='white', linewidth=2))
bp['boxes'][0].set_facecolor(RED)
bp['boxes'][1].set_facecolor(LIGHT)
ax8.set_title("Distribusi Kolesterol per Kelas", fontweight='bold', color=RED)
ax8.set_ylabel("Kolesterol")
ax8.set_facecolor(CREAM)
ax8.spines['top'].set_visible(False)
ax8.spines['right'].set_visible(False)

# ── Plot 9: Tabel Ringkasan Hasil ─────────────────────────────────────────────
...
plt.savefig("modifikasi2_hasil.png", dpi=150, bbox_inches='tight',
            facecolor=CREAM)
print("\n[7] Visualisasi disimpan: modifikasi2_hasil.png")
plt.show()